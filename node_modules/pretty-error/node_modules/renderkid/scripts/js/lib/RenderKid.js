...
module.exports = RenderKid = (function() {
  ...
  RenderKid.prototype._renderDom = function(dom) {
    ...
    this._renderBlockNode(bodyTag, rootBlock);
    return layout.get();
  };

  RenderKid.prototype._renderChildren = function(nodes, parentBlock) {
    var node, _i, _len;
    for (_i = 0, _len = nodes.length; _i < _len; _i++) {
      node = nodes[_i];
      this._renderNode(node, parentBlock);
    }
  };

  RenderKid.prototype._renderNode = function(node, parentBlock) {
    if (node.type === 'text') {
      this._renderText(node, parentBlock);
    } else if (node.name === 'br') {
      this._renderBr(node, parentBlock);
    } else if (this._isBlock(node)) {
      this._renderBlockNode(node, parentBlock);
    } else if (this._isNone(node)) {
      return;
    } else {
      this._renderInlineNode(node, parentBlock);
    }
  };

  RenderKid.prototype._renderText = function(node, parentBlock) {
    var text;
    text = node.data;
  ...
  RenderKid.prototype._renderBlockNode = function(node, parentBlock) {
    ...
    this._renderChildren(node.children, block);
    if (after !== '') {
      block.write(after);
    }
    return block.close();
  ...
  RenderKid.prototype._renderInlineNode = function(node, parentBlock) {
    ...
    this._renderChildren(node.children, parentBlock);
    if (after !== '') {
      return parentBlock.write(after);
    }
  };

  RenderKid.prototype._renderBr = function(node, parentBlock) {
